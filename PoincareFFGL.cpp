...
static CFFGLPluginInfo PluginInfo ( 
	PoincareFFGL::CreateInstance,	// Create method
	"POIN",								// Plugin unique ID
	"Poincare",					// Plugin name
	1,						   			// API major version number
	500,								  // API minor version number
	1,										// Plugin major version number
	000,									// Plugin minor version number
	FF_EFFECT,						// Plugin type
	"Poincare Kaleidoscope",	// Plugin description
	"by Blake Courter (blakecourter.com) Ported to FFGL by Michael Dewberry (dewb.org)" // About
);
...
DWORD PoincareFFGL::InitGL(const FFGLViewportStruct *vp)
{
...
DWORD PoincareFFGL::ProcessOpenGL(ProcessOpenGLStruct *pGL)
{
...
DWORD PoincareFFGL::GetParameter(DWORD dwIndex)
{
	DWORD dwRet;
...
DWORD PoincareFFGL::SetParameter(const SetParameterStruct* pParam)
{
	if (pParam != NULL) {
